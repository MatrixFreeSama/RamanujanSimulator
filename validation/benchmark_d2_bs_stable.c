...
static int stable_case(uint64_t digits, int reps) {
    ...
    /* One untimed correctness pass with real files. */
    double junk = 0.0;
    unsigned long nc = 0UL, nd = 0UL;
    if (!chud_bs_pi(digits, guard, ref_path, &junk, &nc) ||
        !d2_bs_pi(digits, guard, d2_path, &junk, &nd) ||
        !files_equal(ref_path, d2_path)) {
        return 0;
    }

    /* Warm both paths once. /dev/null retains decimal conversion cost while
       removing filesystem-write latency from the timed samples. */
    ...
    for (int r = 0; r < reps; ++r) {
        ...
        /* Alternate order to reduce systematic thermal / scheduler drift. */
        if ((r & 1) == 0) {
