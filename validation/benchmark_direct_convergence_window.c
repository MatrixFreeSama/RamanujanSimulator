 ...
 *
 * This deliberately stays on the original scalar/direct evaluator path.
 * It does NOT use binary splitting and it does NOT carry the quadratic field
 * through the series.  It accelerates the direct recurrence in two ways:
 *
 *  1. Tapered precision.  Since term n is about 10^(-q n), its relative
 *     precision requirement falls by about q decimal digits per step.
 *  2. Magnitude-window accumulation.  Terms are summed into a short local
 *     accumulator whose precision only spans one convergence window, then
 *     that block is added once to the full-precision F and theta-F sums.
 *
 * The purpose is to preserve the formula-level digits/term advantage of D2
 * rather than replacing the direct path by an exact algebraic product tree.
 */
...
static void cw_series(const mpf_t z,
    ...
    if (block_terms == 0UL) block_terms = 64UL;

    for (unsigned long a = 1UL; a < terms; a += block_terms) {
        unsigned long b = a + block_terms;
        if (b > terms) b = terms;
        unsigned long len = b - a;
        mp_bitcnt_t wbits = cw_window_bits(digits_per_term, len, bits);

        mpf_set_prec_raw(bF, wbits);
        mpf_set_prec_raw(bT, wbits);
        ...
        /* Only one full-target-precision accumulation per convergence window. */
        mpf_add(F, F, bF);
        mpf_add(T, T, bT);
    }

    /* mpf_set_prec_raw() requires restoring the allocated precision before clear. */
    mpf_set_prec_raw(term, bits);
    mpf_set_prec_raw(zwork, bits);
    mpf_set_prec_raw(bF, bits);
    mpf_set_prec_raw(bT, bits);
    mpf_set_prec_raw(nt, bits);
...
static int run_cw(uint64_t digits, unsigned long block_terms) {
    ...
    char ref[256], db[256], dcw[256], cb[256], ccw[256];
    ...
    double tr=0, tdb=0, tdcw=0, tcb=0, tccw=0;
    unsigned long nr=0, ndb=0, ndcw=0, ncb=0, nccw=0;
    if (!chud_bs_pi(digits, guard, ref, &tr, &nr) ||
        ...
        !chud_cw_pi(digits, guard, block_terms, ccw, &tccw, &nccw)) return 0;

    int eq_db = files_equal(ref, db);
    int eq_dcw = files_equal(ref, dcw);
    int eq_cb = files_equal(ref, cb);
    int eq_ccw = files_equal(ref, ccw);

    printf("%llu,%lu,%.9f,%.9f,%.9f,%.9f,%.9f,%.6f,%.6f,%d,%d,%d,%d\n",
           (unsigned long long)digits, block_terms,
           tr, tdb, tdcw, tcb, tccw,
           tdcw/tdb, tdcw/tccw,
           eq_db, eq_dcw, eq_cb, eq_ccw);
    fflush(stdout);
    return eq_db && eq_dcw && eq_cb && eq_ccw;
}
...
int main(int argc, char **argv) {
    ...
    unsigned long block_terms = strtoul(argv[2], NULL, 10);
    ...
    if (digits == 0ULL || block_terms == 0UL) return 2;
    return run_cw(digits, block_terms) ? 0 : 1;
}
